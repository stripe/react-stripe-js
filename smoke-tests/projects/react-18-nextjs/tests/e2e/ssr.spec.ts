...
test.describe('SSR + Hydration (React 18 + Next.js 14)', () => {
  ...
  test('checkout page: submit button becomes enabled when stripe is ready', async ({page}) => {
    ...
    await expect(page.locator('[data-testid="submit-btn"]')).toHaveText('Pay now', {timeout: 5000});
  });
  ...
  test('checkout page: no console errors during full page lifecycle', async ({page}) => {
    ...
    const nonStripeErrors = allErrors.filter(e =>
      !e.includes('stripe') && !e.includes('Stripe') // ignore CDN load failures in test
    );
